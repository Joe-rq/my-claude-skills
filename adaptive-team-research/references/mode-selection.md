...
> **Workflow 化后的切换语义（v1.4.0 起）：** 模式在 Phase 0 由主会话确定为单一 `mode` 传入 Workflow，运行中不支持动态改拓扑。中途切换 = 主会话重选模式 + 在画布"模式切换日志"记录 + 重新组装 `args` 调用 Workflow（Round 1-2 重跑）。"先对等后集中"这类混合方案无需重跑——对等模式 Workflow 跑完 Round 1-2 拿到结果后，主会话在 Round 3 直接用集中式裁决逻辑收敛（Round 3 本就在主会话，可自由选择裁决风格）。详见 `workflow-orchestration.md` §8。
